...
class PullRequestCommand : BaseCommand() {
    ...
    private val github = GitHubClient("hannibal002", "SkyHanni", DiscordBot.config.githubToken)
    ...
    override fun MessageReceivedEvent.execute(args: List<String>) {
        if (args.size != 2) {
            userError("Usage: `!pr <number>`")
            return
        }
        val prNumber = args[1].toIntOrNull() ?: run {
            userError("unknown number \uD83E\uDD7A (${args[1]})")
            return
        }
        if (prNumber < 1) {
            userError("PR number needs to be positive \uD83E\uDD7A")
            return
        }
...
//        reply("$title$time$artifactDisplay")
    ...
    private fun MessageReceivedEvent.pullRequestArtifactCommand(args: List<String>) {
        if (args.size != 2) {
            userError("Usage: `!prupload <number>`")
            return
        }
        val prNumber = args[1].toIntOrNull() ?: run {
            userError("Unkown number \uD83E\uDD7A (${args[1]})")
            return
        }
